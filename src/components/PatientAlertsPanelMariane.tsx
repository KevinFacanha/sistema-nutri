...
interface PatientAlertsPanelMarianeProps {
  patients: PatientMariane[];
  onViewAllOverdue: () => void;
  onMarkAsContacted: (patientId: string, milestone: number) => Promise<void>;
}

export const PatientAlertsPanelMariane: React.FC<PatientAlertsPanelMarianeProps> = ({ 
  patients, 
  onViewAllOverdue,
  onMarkAsContacted
}) => {
  ...
  const shouldShowInAlerts = (patient: PatientMariane): boolean => {
    const milestones = [7, 15, 30, 45, 60, 90, 120, 180, 360] as const;
    const days = calculateDaysSince(patient.data_consulta);
    ...
    const contactedSameMilestone =
      ...
      new Date(patient.last_contacted_at) > new Date(patient.data_consulta);
  ...
  const getPatientAlerts = (): PatientAlert[] => {
    return patients
      ...
      .map(patient => ({
        patient,
        daysSinceConsultation: calculateDaysSince(patient.data_consulta),
        category: categorizePatient(calculateDaysSince(patient.data_consulta))
      }))
        ...
        <div className="border-t border-gray-200 p-4 space-y-4">
          ...
          <div className="flex items-start gap-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-sm text-blue-800">
              Esses pacientes estão há mais tempo sem retornar. Acompanhe-os de perto para manter o tratamento de micropigmentação eficiente.
            </p>
          </div>
                  ...
                  {categoryAlerts.map(alert => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-800">{alert.patient.nome}</p>
                          <p className="text-sm text-gray-600">{alert.patient.telefone}</p>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() => handleCopyMessage(alert.patient.id, alert.patient.nome, alert.category)}

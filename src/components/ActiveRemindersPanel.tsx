...
export const ActiveRemindersPanel = ({ patients }: ActiveRemindersPanelProps) => {
  ...
  const getActiveReminders = (): ActiveReminder[] => {
    return patients
      ...
      .map(patient => {
        const daysSinceConsultation = calculateDaysSince(patient.last_consultation);
                ...
                {categoryReminders.map(reminder => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-gray-800">{reminder.patient.name}</p>
                        <p className="text-sm text-gray-600">{reminder.patient.phone}</p>
                      </div>
                    </div>
